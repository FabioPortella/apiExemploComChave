...
public class CursoController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<Curso>>> Get()
    {
        try
        {
            return Ok(await context.Cursos.Include(p => p.TipoDoCurso).OrderBy(p => p.Nome).ToListAsync());
        }
        catch
        {
    ...
    [HttpGet("buscarportipocurso/{TipoCursoID}")]
    public async Task<ActionResult<IEnumerable<Curso>>> BuscarPorTipoCurso(int TipoCursoID)
    {
        try
        {
            return Ok(await context.Cursos.Where(p => p.TipoCursoId == TipoCursoID).OrderBy(p => p.Nome).ToListAsync());
        }
        catch
        {
            return BadRequest("Erro ao buscar os cursos");
        }
    }
}